...
class UserAwards:
    ...
    def __init__(self, user: User):
        award_list = UserAward.objects.filter(user=user, active=True).all()
        self.awards: List[UserAward] = list(sorted(award_list, key=lambda x: (100 - UserAwardLevel(x.award_level).int_value, x.award_text)))
