...
def _import_contigs(apps, _schema_editor):
    ...
    for build_name, (alias, igv_genome) in BUILDS_ALIAS_IGV.items():
        create_build_and_contigs(get_model, build_name, alias, igv_genome)
...
def _genome_build_accession(apps, _schema_editor):
    ...
    for build_name, assembly_report in reference_contigs.ASSEMBLY_REPORTS.items():
        ...
        # Not everything in ASSEMBLY_REPORTS is inserted here
        GenomeBuild.objects.filter(pk=build_name).update(accession=accession)
