...
from django.db.models import Count, Q, SET_NULL
...
def _clear_set_null_references(cursor, Variant, bad_seq_pk, min_pk, max_pk):
    """ on_delete is applied by Django's collector, in Python - so bypassing the collector means
        we have to honour it ourselves. SET_NULL is the case that needs work: Postgres sees
        those FKs as NO ACTION and will refuse the delete rather than null the column.

        (That refusal is how this was found - the FKs are DEFERRABLE INITIALLY DEFERRED, so the
        violation only surfaces at COMMIT. analysis_allvariantsnode.max_variant is the one that
        bites on SA Path; nulling it is exactly what the model asks for, and the node treats a
        null max_variant as "out of date", which is true once these variants are gone.)

        CASCADE targets are handled explicitly elsewhere, and PROTECT / DO_NOTHING are left to
        Postgres to enforce. Derived from the model metadata rather than hardcoded so a new
        SET_NULL FK cannot silently reintroduce this. """
    cleared = []
    for rel in Variant._meta.related_objects:
        field = rel.field
        ...
        while chunk_start <= max_pk:
            ...
            cursor.execute(f'''
                ...
                WHERE "{column}" IN (SELECT id FROM snpdb_variant
                                     WHERE alt_id = %s AND id BETWEEN %s AND %s)
            ''', [bad_seq_pk, chunk_start, chunk_end])
            total += cursor.rowcount
            chunk_start = chunk_end + 1
        if total:
            cleared.append(f"{table}.{column} ({total} rows)")
    if cleared:
        print(f"Cleared SET_NULL references to alt='.' variants: {', '.join(cleared)}")
    return cleared


def _delete_reference_alt_variants(cursor, Variant, bad_seq_pk):
    ...
    _clear_set_null_references(cursor, Variant, bad_seq_pk, min_pk, max_pk)
...
def _one_off_reference_variant_fixes(apps, schema_editor):
    ...
    if bad_seq := Sequence.objects.filter(seq='.').first():
        ...
        with schema_editor.connection.cursor() as cursor:
            _delete_reference_alt_variants(cursor, Variant, bad_seq.pk)
