...
- models/models_vcf.py — VCF, Sample, VCFFilter; models/models_cohort.py — Cohort, CohortSample, CohortGenotypeCollection, CohortGenotype
- models/models_family.py — FamilyGroupMixin and the named family structures over a Cohort: Duo, Trio, Quad
...
- Gene-level events (a fusion, or a whole-gene copy number call) are Variants on a fake contig with a gene id for a position; guard coordinate code with models/models_variant.py:Variant.get_gene_level_q and read gene_level_variants.py first. Which kind a Variant is comes off the alt (library/genomics/vcf_enums.py:GeneLevelSymbolicAlt), so each of genes/gene_fusions.py:create_gene_fusions_for_variants and genes/gene_copy_number.py:create_gene_copy_number_events_for_variants claims only its own.
...
- somalier keeps each site's two alleles alphabetically and reads the genotype against that pair rather than against the record's `REF`/`ALT` (brentp/somalier#163), so variants_to_vcf.py:vcf_export_to_file writes the `AD` pair in the site's order - `REF`, `ALT` and `GT` stay as called, and only a VCF with no depths flips `GT` instead. Getting it wrong is invisible in a jointly called VCF - it cancels out pairwise - and shows up as inflated relatedness once `--unknown` is in play (#183). v0.3.5's fix for brentp/somalier#163 is only `relate --sites` correcting the per-sample counts in its samples TSV: pairs are still computed in sorted order, so it still needs the compensation (`settings.SOMALIER["compensate_allele_order"]` stays `True`; turning it off also passes `--sites`, models/models_somalier.py:SomalierConfig.get_relate_sites_args). `deployment_check`'s `somalier_allele_order` relates a hom-alt sample against a no-call one with `--unknown` through the installed binary and fails saying which way to set it - a per-sample count check can't tell, both settings pass one on v0.3.5 (variantgrid/deployment_validation/somalier_check.py). Changing it means rebuilding the extracts.
- An all-samples relate spans genome builds but `relate` takes one sites VCF, so models/models_somalier.py:SomalierAllSamplesRelate.genome_build hands it the build most samples are in. The builds' sites files are the same variants at different coordinates and disagree on alleles at ~0.8% of sites, which are the only ones read the wrong way round.
