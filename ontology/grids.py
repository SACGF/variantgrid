...
class AbstractOntologyGenesGrid(DataFrameJqGrid, abc.ABC):
    ...
    def get_dataframe(self):
        ...
        for hpo in hpo_qs:
            for gene in OntologySnake.special_case_gene_symbols_for_hpo_and_omim(hpo_terms=[hpo]):
                gene_terms_set[gene.symbol]["hpo"].add(str(hpo))

        for omim in omim_qs:
            for gene in OntologySnake.special_case_gene_symbols_for_hpo_and_omim(omim_terms=[omim]):
