...
from variantclassification.models.evidence_mixin import VCPatch, VCStore
...
class VariantClassificationJsonParams:
    """
    When converting VariantClassifications to JSON use these parameters to indicate how.
    """
    def __init__(self,
                 current_user: User,
                 include_data=False,
                 version: Optional[Union['VariantClassificationModification', float]] = None,
                 flatten=False,
...
class VariantCoordinateFromEvidence:
    ...
    """
    def __init__(self, variant_classification: 'VariantClassification'):
        """
        :param variant_classification: The classification we're extracting data from
        """
        from variantclassification.models import variant_classification_flag_types
        self.variant_coordinate = None
        self.messages = []
        self.matching_flag = variant_classification.flag_collection_safe.get_flag_of_type\
            (flag_type=variant_classification_flag_types.matching_variant_flag, open_only=True)
        try:
            genome_build = variant_classification.get_genome_build()
    ...
    def report(self) -> None:
        """
        Record all information about the VariantMatching evidence on the VariantMatching flag of the VariantClassification
        """
