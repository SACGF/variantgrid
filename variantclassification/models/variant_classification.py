...
from variantclassification.models.best_hgvs import BestHGVS
from variantclassification.models.evidence_key import EvidenceKeyValueType, \
    EvidenceKey, EvidenceKeyMap, VCDataDict, WipeMode, VCDataCell
from variantclassification.models.evidence_mixin import EvidenceMixin, VCPatch
from variantclassification.models.flag_types import variant_classification_flag_types
from variantclassification.models.variant_classification_utils import \
    ValidationMerger, VariantClassificationJsonParams, VariantCoordinateFromEvidence, PatchMeta
from variantclassification.regexes import db_ref_regexes, DbRegexes

ChgvsKey = namedtuple('CHGVS', ['short', 'column', 'build'])

variant_classification_validation_signal = django.dispatch.Signal(providing_args=["variant_classification", "patch_meta", "key_map"])
variant_classification_current_state_signal = django.dispatch.Signal(providing_args=["user"])
variant_classification_post_publish_signal = django.dispatch.Signal(providing_args=["variant_classification", "previously_published", "previous_share_level", "newly_published", "user"])
variant_classification_withdraw_signal = django.dispatch.Signal(providing_args=["variant_classification", "user"])
variant_classification_variant_set_signal = django.dispatch.Signal(providing_args=["variant_classification", "variant"])
variant_classification_revalidate_signal = django.dispatch.Signal(providing_args=["variant_classification"])
...
class VariantClassificationProcessError(Exception):
    """
...
class CreateNoVariantClassificationForbidden(PermissionDenied):
    """ Used if settings prevent creating classifications not linked to a variant """
    pass


class VariantClassificationImport(models.Model):
    ...
    def get_variants_qs(self):
        q_classification_variants = Q(variantclassification__variant_classification_import=self)
        return Variant.objects.filter(q_classification_variants).distinct()


class VariantClassificationImportAlleleSource(AlleleSource):
    """ A model to indicate that variants need to be linked to an allele and lifted over to other builds """
    variant_classification_import = models.ForeignKey(VariantClassificationImport, null=True, on_delete=CASCADE)

    def get_allele_qs(self):
        variants_qs = self.variant_classification_import.get_variants_qs()
        return Allele.objects.filter(variantallele__variant__in=variants_qs)

    def liftover_complete(self, genome_build: GenomeBuild):
        VariantClassification.bulk_update_cached_c_hgvs(self.variant_classification_import)
        allele: Allele
        for allele in self.get_allele_qs():
            allele.validate()
        report_event('Completed import liftover', extra_data={'liftover_id': self.pk, 'allele_count': self.get_allele_qs().count()})


class AllClassificationsAlleleSource(TimeStampedModel, AlleleSource):
    """ Used to reload all VariantClassifications (for upgrades etc) """
    ...
    def get_variants_qs(self):
        ...
        # ie we don't use VariantClassification.get_variant_q_from_classification_qs() to get liftovers
        contigs_q = Variant.get_contigs_q(self.genome_build)
        return Variant.objects.filter(contigs_q, variantclassification__isnull=False)

    def get_allele_qs(self):
        return Allele.objects.filter(variantallele__variant__in=self.get_variants_qs())

    def liftover_complete(self, genome_build: GenomeBuild):
        VariantClassification.bulk_update_cached_c_hgvs()
        allele: Allele
        for allele in self.get_allele_qs():
            allele.validate()
...
def get_extra_info(flag_infos: FlagInfos, user: User, **kwargs) -> None:
    """
    Allows us to provide extra info for FlagCollections attached to VariantClassification
    ...
    from variantclassification.enums.discordance_enums import DiscordanceReportResolution

    vcs = VariantClassification.objects.filter(flag_collection__in=flag_infos.ids).select_related('lab')
    drcs = DiscordanceReportClassification.objects.filter(classification_original__variant_classification__in=vcs, report__resolution=DiscordanceReportResolution.ONGOING)\
        .values_list('classification_original__variant_classification', 'report')
    drcs_dict = {}
    for drc in drcs:
        drcs_dict[drc[0]] = drc[1]

    def find_unsubmitted(flags: List[Flag]) -> Optional[Flag]:
        # return outstanding edits if we have it, otherwise fall back to unshared flag
        for flag in flags:
            if flag.flag_type == variant_classification_flag_types.variant_classification_outstanding_edits:
                return flag
        for flag in flags:
            if flag.flag_type == variant_classification_flag_types.unshared_flag:
                return flag
...
class VariantClassification(GuardianPermissionsMixin, FlagsMixin, EvidenceMixin, TimeStampedModel):
    """
    A Variant Classification, belongs to a lab and user. Keeps a full history using VariantClassificationModification
    ...
    :cvar variant_classification_import: provide a value here to later match this record to a variant
    ...
    :cvar evidence: The latest evidence (should always match the content of the latest VariantClassificationModification.evidence)
    ...
    @staticmethod
    def bulk_update_cached_c_hgvs(vc_import: Optional[VariantClassificationImport] = None):
        """
        ...
        requires_caching = VariantClassification.objects.filter(
            (tests | Q(clinical_context__isnull=True)) & Q(variant__isnull=False)
        )
        if vc_import:
            requires_caching = requires_caching.filter(variant_classification_import=vc_import)
    ...
    def __init__(self, *args, **kwargs):
        super(VariantClassification, self).__init__(*args, **kwargs)
        self.requires_auto_population = False

    # Here to make this interchangeable with a VariantClassificationRef
    @property
    def record(self):
        return self
    ...
    def set_withdrawn(self, user: User, withdraw: bool = False) -> bool:
        ...
        if withdraw:
            self.flag_collection_safe.get_or_create_open_flag_of_type(
                flag_type=variant_classification_flag_types.variant_classification_withdrawn,
                user=user,
        ...
        variant_classification_withdraw_signal.send(VariantClassification, variant_classification=self)
        return True
    ...
    def update_cached_c_hgvs(self) -> int:
        ...
        if self.variant:
            if GenomeBuild.grch37().is_annotated:
                ...
                self.chgvs_grch37_full = c_hgvs_name.format(max_allele_length=settings.VARIANT_CLASSIFICATION_MAX_FULL_ALLELE_LENGTH)
            if GenomeBuild.grch38().is_annotated:
                c_hgvs_name = self._generate_c_hgvs_extra(genome_build=GenomeBuild.grch38())
                max_length = max(c_hgvs_name.ref_lengths(), max_length)
                self.chgvs_grch38 = c_hgvs_name.format()
                self.chgvs_grch38_full = c_hgvs_name.format(max_allele_length=settings.VARIANT_CLASSIFICATION_MAX_FULL_ALLELE_LENGTH)
        else:
    ...
    def set_variant(self, variant: Variant = None, message: str = None, failed: bool = False):
        ...
        If there is no variant and failed = True, any variant_classification_import will be unset
        """
        self.chgvs_grch37 = None
        self.chgvs_grch38 = None
        ...
        if variant is None and failed:
            self.variant_classification_import = None
        ...
        try:
            ...
            if variant:
                ...
                flag_collection.close_open_flags_of_type(variant_classification_flag_types.matching_variant_flag, comment='Variant Matched')
                variant_classification_variant_set_signal.send(sender=VariantClassification, variant_classification=self, variant=variant)
                ...
                if transcript_comment:
                    flag_collection.get_or_create_open_flag_of_type(
                        flag_type=variant_classification_flag_types.transcript_version_change_flag,
                        comment=transcript_comment,
                        only_if_new=True
                    )
                else:
                    flag_collection.close_open_flags_of_type(variant_classification_flag_types.transcript_version_change_flag)

                if matching_warning_comment:
                    flag_collection.get_or_create_open_flag_of_type(
                        flag_type=variant_classification_flag_types.matching_variant_warning_flag,
                        comment=matching_warning_comment,
                        only_if_new=True
                    )
                else:
                    flag_collection.close_open_flags_of_type(variant_classification_flag_types.matching_variant_warning_flag)
                ...
                flag_collection.ensure_resolution(variant_classification_flag_types.matching_variant_flag,
    ...
    def filter_for_user(cls, user, queryset=None, **kwargs):
        """ VariantClassification only has write permission, View is based on a version in a point at time
            see VariantClassificationModification's read permission """
    ...
    def create(user: User,
               ...
               **kwargs) -> 'VariantClassification':
        """
            :param user: The user creating this VariantClassification
        ...
        record = VariantClassification(user=user,
                                       lab=lab,
    ...
    def save(self, **kwargs):
        """ The post_save event is fired after save(), for other logic that depends on VariantClassifications
            but doesn't work with the VariantClassification object itself.
        """
    ...
    def process_option_values(cell: VCDataCell, values: List[Any]) -> Optional[List[str]]:
        ...
        for check_value in values:
            ...
            check_value = str(check_value)
            matched_option = VariantClassification.match_option(options, check_value)
            if matched_option:
    ...
    def process_entry(self, cell: VCDataCell, source: str):
        ...
        if value is not None:
            ...
            if e_key.key == 'existing_variant_id':
                try:
                    ...
                    cell.add_validation(code=ValidationCode.MATCHING_ERROR, severity='error', message="Couldn't resolve Variant " + str(value) + ")")
                ...
                try:
                    ...
                    cell.add_validation(code=ValidationCode.MATCHING_ERROR, severity='error', message="Couldn't resolve Sample " + str(value) + ")")
            ...
            # otherwise leave the value as is to be caught by validation
                ...
                if not isinstance(value, bool):
                    if value == 1:
                        ...
                        if value in ('TRUE', 'Y', 'YES', 'T', '1'):
                            ...
                            cell.add_validation(code=ValidationCode.INVALID_VALUE, severity='error', message="Invalid value for yes/no field (" + str(value) + ")")
                cell.value = value
                ...
                value = VariantClassification.process_option_values(cell, parts)
                if value is not None:
                    if e_key.value_type != EvidenceKeyValueType.MULTISELECT:
                        value_len = len(value)
                        if value_len > 1:
                            cell.add_validation(code=ValidationCode.TOO_MANY_VALUES, severity='error', message="Can only have a single value for a select field")
                        elif value_len == 1:
                            value = value[0]
                            cell.value = value
                cell.value = value
                ...
                try:
                    ...
                    pass
                if not valid:
                    cell.add_validation(code=ValidationCode.INVALID_PERCENT, severity='warning', message='Value should be a number between 0 and 100')
                ...
                try:
                    ...
                    pass
                if not valid:
                    cell.add_validation(code=ValidationCode.INVALID_UNIT, severity='warning', message='Value should be a number between 0 and 1')
                ...
                try:
                    ...
                    pass
                if not valid:
                    cell.add_validation(code=ValidationCode.INVALID_FLOAT, severity='warning', message='Value should be a number')
                ...
                try:
                    VariantClassification.to_date(value)
        ...
        if e_key.key == SpecialEKeys.REFSEQ_TRANSCRIPT_ID and isinstance(value, str):
            ...
            if not version_regex.match(value):
                cell.add_validation(code=ValidationCode.UNKNOWN_TRANSCRIPT, severity='warning', message='Transcript should include version e.g. NM_001256799.1')

        # ensure we have one non None value before returning the structure
        if not cell.has_data:
            cell.wipe(WipeMode.SET_NONE)

    def modification_at_timestamp(self, version_timestamp) -> 'VariantClassificationModification':
        dt = datetime.utcfromtimestamp(version_timestamp).replace(tzinfo=timezone.utc)
        vcm = VariantClassificationModification.objects.filter(variant_classification=self, created__lte=dt).order_by('-created').first()
        return vcm
    ...
    def revalidate(self, user: User):
        ...
        variant_classification_revalidate_signal.send(sender=VariantClassification, variant_classification=self)
    ...
    def fix_migration_stuff(self, user: User) -> None:
        ...
        """
        modifications_for_self_qs = VariantClassificationModification.objects.filter(variant_classification=self)
    ...
    def patch_history(self, patcher: Callable[[PatchMeta], None]):
        ...
        """
        last_evidence = {}
        modification: VariantClassificationModification
        for modification in VariantClassificationModification.objects.filter(variant_classification=self).order_by('created'):
    ...
    def patch_value(self,
        ...
        """
            Creates a new VariantClassificationModification if the patch values are different to the current values
        ...
        for cell in patch.cells():
            if cell.raw is not None:
                ...
                # Don't set immutability to form as that's the loweest immutability level
        ...
        validations_received = variant_classification_validation_signal.send(sender=VariantClassification, variant_classification=self, patch_meta=patch_meta, key_map=key_dict)
        ...
        if apply_patch:
            ...
            if last_edited and last_edited.is_edit_appendable(user=user, source=source):
                ...
                pending_modification = VariantClassificationModification(
                    user=user,
            ...
            ###
            VariantClassification.patch_with(target=use_evidence.data, patch=diffs_only_patch)
            ...
            warnings.append({'code': 'patched', 'message': 'Patched changed values for ' + ', '.join(diffs_only_patch.keys())})
            ...
            if save:
                ...
                if pending_modification:
                    pending_modification.variant_classification = self
                    pending_modification.save()
                    assign_perm(VariantClassificationModification.get_read_perm(), self.lab.group, pending_modification)
        ...
        if self.requires_auto_population:
            self.requires_auto_population = False
            from variantclassification.autopopulate_evidence_keys.autopopulate_evidence_keys import variant_classification_auto_populate_fields
            genome_build = self.get_genome_build()
            variant_classification_auto_populate_fields(self, genome_build, save=save)
    ...
    @property
    def last_edited_version(self) -> 'VariantClassificationModification':
        return VariantClassificationModification.objects.filter(variant_classification=self, is_last_edited=True).first()

    def latest_modification_for_user(self, user: User, exclude_withdrawn: bool = True) -> 'VariantClassificationModification':
        return VariantClassificationModification.latest_for_user(user, variant_classification=self, exclude_withdrawn=exclude_withdrawn).first()

    @property
    def last_published_version(self) -> 'VariantClassificationModification':
        return VariantClassificationModification.objects.filter(variant_classification=self, is_last_published=True).first()
    ...
    def current_state_validation(self, user: User) -> ValidationMerger:
        ...
        validations_received = variant_classification_current_state_signal.send(sender=VariantClassification, record=self, user=user)
        multi_errors = ValidationMerger.union_send_responses(validations_received)
    ...
    def validate(self) -> List[Dict]:
        return VariantClassification.validate_evidence(self.evidence)
    ...
    def flatten(record: Mapping, prefix: str = '', ignore_none_values=False, depth=0) -> dict:
        ...
        for key, value in record.items():
            ...
            if isinstance(value, Mapping):
                flat.update(VariantClassification.flatten(value, prefix=use_key, depth=depth + 1))
            elif key == 'value':
                if isinstance(value, list):
                    if value:
                        value = ', '.join(value)
    ...
    def as_json(self,
        ...
        if version and not isinstance(version, VariantClassificationModification):
        ...
        if include_messages:
            content['messages'] = VariantClassification.validate_evidence(self.evidence)
        ...
        if version is None or version.is_last_edited:
            ...
            if include_messages:
                content['messages'] = VariantClassification.validate_evidence(versioned_data)
            content['can_write'] = False

        if include_lab_config:
            content['config'] = EvidenceKey.merge_config(self.lab.variant_classification_config, self.lab.organization.variant_classification_config)
        ...
        if flatten:
            data = VariantClassification.flatten(data, ignore_none_values=True)
    ...
    def _get_variant_coordinates_from_evidence(self) -> VariantCoordinateFromEvidence:
        ...
        if variant_coordinate:
            if SpecialEKeys.VARIANT_COORDINATE in key_errors:
                vcfe.record(variant_coordinate, error=key_errors[SpecialEKeys.VARIANT_COORDINATE].get('message', 'Validation Error'))
            else:
                try:
    ...
    def get_absolute_url(self):
        return reverse('view_variant_classification', kwargs={'record_id': self.pk})
    ...
    def get_variant_q(user: User, genome_build: GenomeBuild, clinical_significance_list: Iterable[str] = None) -> Q:
        ...
        vcm_qs = VariantClassificationModification.latest_for_user(user, published=True)
        if clinical_significance_list:
            vcm_qs = vcm_qs.filter(clinical_significance__in=clinical_significance_list)
        vc_qs = VariantClassification.objects.filter(pk__in=vcm_qs.values('variant_classification'))
        return VariantClassification.get_variant_q_from_classification_qs(vc_qs, genome_build)
    ...
    def get_variant_q_from_classification_qs(vc_qs, genome_build: GenomeBuild) -> Q:
        ...
        return Q(id__in=list(variant_ids))  # List faster than inner query
    ...
    def get_other_classifications_summary_for_variant(self, user: User) -> str:
        ...
        if self.variant and self.variant.allele:
            latest_others_for_variant = VariantClassificationModification.latest_for_user(user=user, allele=self.variant.allele, published=True)
            latest_others_for_variant = latest_others_for_variant.exclude(variant_classification__id=self.id)

            counts = Counter([vcm.get(SpecialEKeys.CLINICAL_SIGNIFICANCE, "Unclassified") for vcm in latest_others_for_variant])
            classifications = []
    ...
    @staticmethod
    def check_can_create_no_variant_classification_via_web_form(_user: User):
        if not settings.VARIANT_CLASSIFICATION_WEB_FORM_CREATE_ALLOW_NO_VARIANT:
            raise CreateNoVariantClassificationForbidden()


class VariantClassificationModification(GuardianPermissionsMixin, EvidenceMixin, models.Model):
    variant_classification = models.ForeignKey(VariantClassification, on_delete=CASCADE)
    user = models.ForeignKey(User, on_delete=PROTECT)  # One who did last change, may not be variant_classification.user
    ...
    def column_name_for_build(genome_build: GenomeBuild, use_full: bool = False):
        if genome_build.is_equivalent(GenomeBuild.grch37()):
            return 'variant_classification__chgvs_grch37' if not use_full else 'variant_classification__chgvs_grch37_full'
        if genome_build.is_equivalent(GenomeBuild.grch38()):
            return 'variant_classification__chgvs_grch38' if not use_full else 'variant_classification__chgvs_grch38_full'
    ...
    def share_level_enum(self) -> ShareLevel:
        ...
        for sl in [ShareLevel.PUBLIC,
                   ...
                   ShareLevel.LAB]:
            if self.can_view(sl.group(lab=self.variant_classification.lab)):
                return sl
        return ShareLevel.CURRENT_USER
    ...
    @property
    def id_str(self) -> str:
        return self.variant_classification.id_str + '.' + str(self.created.timestamp())

    @property
    def curated_date(self) -> datetime:
        try:
            curated_date_value = VariantClassification.to_date(self.get(SpecialEKeys.CURATION_DATE))
        ...
        return self.variant_classification.created.replace(tzinfo=None)
    ...
    def optimize_for_report(qs: QuerySet) -> QuerySet:
        return qs.select_related(
            ...
            'variant_classification__lab__organization',
            'variant_classification__user',
            'variant_classification__variant',
            'variant_classification__variant__locus',
            'variant_classification__clinical_context')

    def as_json(self,
                params: VariantClassificationJsonParams) -> dict:

        params.version = self
        return self.variant_classification.as_json(params)

    def get_absolute_url(self):
        return reverse('view_variant_classification', kwargs={'record_id': str(self.variant_classification.id) + '.' + str(self.created.timestamp())})

    def __init__(self, *args, **kwargs):
        super(VariantClassificationModification, self).__init__(*args, **kwargs)
        self.cached_evidence = None

    @classmethod
    def latest_for_user(cls,
                        user: User,
                        variant_classification: Optional[VariantClassification] = None,
        ...
        :param variant_classification: If provided will only get modifications for this classification
        ...
        qs = qs.order_by('-created', 'variant_classification')
        if kwargs:
            qs = qs.filter(**kwargs)

        if variant_classification:
            qs = qs.filter(variant_classification=variant_classification)
        if variant is not None:
            if isinstance(variant, (Variant, int)):
                qs = qs.filter(variant_classification__variant=variant)
            else:
                qs = qs.filter(variant_classification__variant__in=variant)
        if allele:
            qs = qs.filter(variant_classification__variant__in=allele.variants)
        if clinical_context:
            qs = qs.filter(variant_classification__clinical_context=clinical_context)
        if exclude_withdrawn:
            qs = qs.filter(variant_classification__withdrawn=False)
        if shared_only:
            qs = qs.filter(share_level__in=ShareLevel.DISCORDANT_LEVEL_KEYS)

        if published:
            qs = qs.select_related('variant_classification', 'variant_classification__lab', 'variant_classification__variant', 'variant_classification__user')
            qs = qs.filter(is_last_published=True)
            return qs
        qs = qs.values_list('id', 'variant_classification')

        seen_variant_classifications = set()
        latest_version_ids = set()
        for (vcm_id, vc_id) in qs:
            if vc_id not in seen_variant_classifications:
                latest_version_ids.add(vcm_id)
                seen_variant_classifications.add(vc_id)
        return VariantClassificationModification.objects.filter(id__in=latest_version_ids, **kwargs)
    ...
    @transaction.atomic()
    def publish(self, share_level: Union[ShareLevel, str, int], user: User, vc: 'VariantClassification') -> bool:
        """
        ...
        lab = self.variant_classification.lab
        ...
        vc.save()
        variant_classification_post_publish_signal.send(
            sender=VariantClassification,
    ...
    @lazy
    def previous(self) -> Optional['VariantClassificationModification']:
        return VariantClassificationModification.objects.filter(variant_classification=self.variant_classification, created__lt=self.created).order_by('-created').first()

    @property
    def index(self):
        return VariantClassificationModification.objects.filter(variant_classification=self.variant_classification, created__lt=self.created).count()
    ...
    def evidence(self) -> Dict[str, Dict]:
        if self.cached_evidence is None:
            if self.published_evidence is not None:
                ...
                for vcm in VariantClassificationModification.objects.filter(variant_classification=self.variant_classification, created__lte=self.created)\
                ...
                for patch in patches_since_evidence:
                    VariantClassification.patch_with(target=data, patch=patch)
                self.cached_evidence = data
        return self.cached_evidence

    def get_visible_evidence(self, user: User) -> Dict[str, Dict]:
        """ Driven by EvidenceKey.max_share_level """
        lowest_share_level = self.variant_classification.lowest_share_level(user)
        return self.variant_classification.get_visible_evidence(self.evidence, lowest_share_level)

    def best_hgvs(self, genome_build: GenomeBuild) -> BestHGVS:
        return self.variant_classification.best_hgvs(genome_build=genome_build)

    def is_significantly_equal(self, other: 'VariantClassificationModification', care_about_explains: bool = False) -> bool:
        """
        ...
        """
        if self.variant_classification_id != other.variant_classification_id:
...
class VariantClassificationConsensus:

    def __init__(self, variant: Variant, user: User, copy_from: Optional[VariantClassificationModification] = None):
        variants = [variant]
        ...
        if copy_from:
            ...
            self.vcm: Optional[VariantClassificationModification] = None
            if allele:
                variants = allele.variants
            vcms = list(VariantClassificationModification.latest_for_user(user=user, variant=variants, published=True).all())
            if vcms:
