...
from variantclassification.models import flag_types, VariantClassification, DiscordanceReport
...
def check_for_discordance(sender, flag_comment: FlagComment, old_resolution: FlagResolution, **kwargs):
    if settings.DISCORDANCE_ENABLED:
        ...
        if resolution:
            ...
            if flag_type == flag_types.variant_classification_flag_types.discordant:
                vc = VariantClassification.objects.filter(flag_collection=flag.collection.id).select_related('lab').get()
...
def email_discordance_for_classification(vc: VariantClassification) -> bool:
    discordant_flag = vc.flag_collection.get_flag_of_type(flag_type=flag_types.variant_classification_flag_types.discordant, open_only=False)
    if discordant_flag:
        ...
        context = {
            ...
            "review_link": get_url_from_view_path(
                reverse('view_variant_classification', kwargs={'record_id': vc.id})),
        }

        html = render_to_string('variantclassification/emails/discordance_email.html', context)
        text = render_to_string('variantclassification/emails/discordance_email.txt', context)
