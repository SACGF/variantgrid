...
from variantclassification.enums import SpecialEKeys, ShareLevel
...
from variantclassification.models.evidence_key import EvidenceKey, EvidenceKeyMap
...
from variantclassification.models.variant_classification import VariantClassification, \
    variant_classification_current_state_signal, \
    variant_classification_post_publish_signal, \
    variant_classification_variant_set_signal, VariantClassificationModification, \
    variant_classification_withdraw_signal
from variantclassification.models.variant_classification_utils import ValidationMerger


INTERNAL_REVIEW_RELEVANT_DAYS = 365


@receiver(variant_classification_variant_set_signal, sender=VariantClassification)
def variant_set(sender, **kwargs):
    record: VariantClassification = kwargs['variant_classification']
...
@receiver(post_delete, sender=VariantClassification)
def deleted_variant(sender, instance: VariantClassification, *args, **kwargs):
    variant_classification = instance
    if variant_classification.clinical_context:
        cause = f'Classification {instance.friendly_label} deleted'
        variant_classification.clinical_context.recalc_and_save(cause=cause)


@receiver(variant_classification_withdraw_signal, sender=VariantClassification)
def withdraw_changed(sender, variant_classification: VariantClassification, **kwargs):
    if variant_classification.clinical_context:
        cause = ''
        if variant_classification.withdrawn:
            cause = f'Classification {variant_classification.friendly_label} withdrawn'
        else:
            cause = f'Classification {variant_classification.friendly_label} un-withdrawn'
...
def published(sender,
              ...
              previously_published: VariantClassificationModification,
              newly_published: VariantClassificationModification,
    ...
    cs = EvidenceKey.objects.get(pk=SpecialEKeys.CLINICAL_SIGNIFICANCE).pretty_value(
        variant_classification.evidence.get(SpecialEKeys.CLINICAL_SIGNIFICANCE)) or 'Unclassified'
    ...
    if first_publish or change_share_level or diff_keys or at_least_minor_diffs:
        ...
        comment_to = f'to {new_share_level.context_label(variant_classification)}' if change_share_level else None
        ...
        variant_classification.flag_collection_safe.add_flag(
            flag_type=variant_classification_flag_types.submitted_flag,
            user=user,
            permission_check=False,
            comment=comment
        )

    if variant_classification.variant and not variant_classification.clinical_context:
        variant_classification.clinical_context = ClinicalContext.default_group_for(variant_classification.variant)
        variant_classification.save(update_fields=['clinical_context'])

    if variant_classification.clinical_context:
        cause = ''
        if first_publish:
            cause = f'Classification {variant_classification.friendly_label} submitted'
        else:
            cause = f'Classification {variant_classification.friendly_label} re-submitted as {cs}'
...
def clinical_context_update(sender, clinical_context: ClinicalContext, status: str, is_significance_change: bool, cause: str, *args, **kwargs):
    if settings.DISCORDANCE_ENABLED:
        ...
        for vc in VariantClassification.objects.filter(clinical_context=clinical_context):
            discordant = vc.id in discordant_classifications
            if discordant:
                vc.flag_collection_safe.get_or_create_open_flag_of_type(
                    flag_type=variant_classification_flag_types.discordant
                )
            else:
...
def apply_classification_flags(sender, clinical_context: ClinicalContext, is_significance_change: bool, *args, **kwargs):
    variant_classifications_modifications = clinical_context.variant_classification_modifications
    ...
    for vc in VariantClassification.objects.filter(clinical_context=clinical_context).exclude(share_level__in = ShareLevel.DISCORDANT_LEVEL_KEYS):
        fc = vc.flag_collection_safe
        fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.internal_review, comment='No longer in discordance')
        fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.discordance_discussion, comment='No longer in discordance')
    
    if not clinical_context.is_discordant():
        for vc in clinical_context.variant_classifications_qs:
            fc = vc.flag_collection_safe
            fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.internal_review, comment='No longer in discordance')
            fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.discordance_discussion, comment='No longer in discordance')
    else:
        ...
        for vcm in variant_classifications_modifications:
            vc = vcm.variant_classification
            fc = vc.flag_collection_safe
            last_closed = Flag.objects.filter(collection=fc, flag_type=variant_classification_flag_types.internal_review).exclude(status=FlagStatus.OPEN)\
                .order_by('-modified').values_list('modified', flat=True).first()
            currently_open = Flag.objects.filter(collection=fc, flag_type=variant_classification_flag_types.internal_review, status=FlagStatus.OPEN).exists()
            if last_closed:
        ...
        for vcm in variant_classifications_modifications:
            ...
            if not recently_reviewed and not currently_in_review:
                ...
                fc.get_or_create_open_flag_of_type(flag_type=variant_classification_flag_types.internal_review, comment=review_required_comment, reopen=False)
            ...
            if recently_reviewed and not currently_in_review and not pause_discussion:
                fc.get_or_create_open_flag_of_type(flag_type=variant_classification_flag_types.discordance_discussion, reopen=True)
            
            # close discussion flags if we're in review or if there's a pause flag on the cc
            if currently_in_review:
                fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.discordance_discussion, comment=f"Discussion paused due to Internal Review")
            elif pause_discussion:
                fc.close_open_flags_of_type(flag_type=variant_classification_flag_types.discordance_discussion, comment=f"Discussion paused due to {pause_discussion.flag_type.label}")
 """


@transaction.atomic
@receiver(variant_classification_current_state_signal, sender=VariantClassification)
def discordance_current_state(sender, **kwargs) -> ValidationMerger:
    """
    Reports if the classification is in discordance or review
    """
    record = kwargs.get('record')  # type: VariantClassification
    ...
    if record.clinical_context and my_classification:
        ...
        qs = qs.exclude(variant_classification=record)
        ...
        for vcm in qs:
            ...
            if other_cs:
                ...
                if not is_same:
                    other_record = vcm.variant_classification
                    ...
                    if record.share_level not in ShareLevel.DISCORDANT_LEVEL_KEYS or \
                        ...
                        different_vcs.append(vcm.variant_classification)
