...
from variantclassification.enums import EvidenceKeyValueType
from variantclassification.models import VariantClassification, EvidenceKey
...
class VariantClassificationTestCaseViews(TestCase):
    ...
    def request_post(self, data):
        _, user = VariantClassificationTestUtils.lab_and_user()
        request = self.requestFactory.get('/variantclassification/api/classifications/v2/record/')
        request.method = 'POST'
        request.content_type = 'application/json'
        request.user = user
        request.data = data
        return VariantClassificationView().post(request=request)

    @override_settings(VARIANT_CLASSIFICATION_MATCH_VARIANTS=False)
    def test_return_data(self):
        lab, _user = VariantClassificationTestUtils.lab_and_user()
