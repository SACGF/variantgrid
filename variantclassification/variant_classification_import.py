...
from upload.models import UploadedFile, UploadPipeline, UploadStep, \
    UploadedVariantClassificationImport
from upload.models_enums import UploadedFileTypes, UploadStepOrigin, \
    UploadStepTaskType, VCFPipelineStage
from upload.upload_processing import process_upload_pipeline
from variantclassification.models.variant_classification import VariantClassificationImport
from variantclassification.tasks.classification_import_process_variants_task import ClassificationImportProcessVariantsTask
...
def process_variant_classification_import(variant_classification_import: VariantClassificationImport, import_source):
    """ VariantClassifications are submitted via API with evidence fields (eg HGVS) which we need to
        ...
        Batch variant classification submissions are broken up into 1 VariantClassificationImport per GenomeBuild """

    from variantclassification.models import VariantClassification
    variant_pk_lookup = VariantPKLookup(variant_classification_import.genome_build)
    ...
    for variant_classification in no_variant_qs:
        try:
            variant_tuple = variant_classification.get_variant_coordinates_from_evidence(update_flags=True)
            if variant_tuple:
                ...
                variant_classification.set_variant(variant=None, message='Could not derive variant coordinates', failed=True)
        except ValueError as ve:
            variant_classification.set_variant(variant=None, message=str(ve), failed=True)
    ...
    for variant_hash, variant_pk in variant_pk_lookup.variant_pk_by_hash.items():
        ...
        if variant_pk:
            variant_classification.set_variant(Variant.objects.get(pk=variant_pk))
        else:
            ...
            if not _is_safe_for_vcf(variant_tuple):
                ...
                variant_classification.set_variant(None, message=f'Could not process via VCF, ref length = {ref_length}', failed=True)
            else:
                unknown_variant_coordinates.append(variant_tuple)

    _classification_upload_pipeline(variant_classification_import, unknown_variant_coordinates, import_source)
...
def _classification_upload_pipeline(
    ...
    if unknown_variant_tuples_list:
        working_dir = get_import_processing_dir(variant_classification_import.pk, "variant_classification_import")
        vcf_filename = os.path.join(working_dir, "variant_classification_import.vcf")
    ...
    uploaded_file = UploadedFile.objects.create(path=vcf_filename,
                                                ...
                                                user=variant_classification_import.user,
                                                file_type=UploadedFileTypes.VCF_INSERT_VARIANTS_ONLY,
                                                visible=False)

    UploadedVariantClassificationImport.objects.create(uploaded_file=uploaded_file,
                                                       variant_classification_import=variant_classification_import)
