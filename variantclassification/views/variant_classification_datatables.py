...
from variantclassification.enums import SpecialEKeys, EvidenceCategory
from variantclassification.models import VariantClassificationModification, variant_classification_flag_types, \
    VariantClassification, EvidenceKeyMap
...
class VariantClassificationDatatableConfig(DatatableConfig):
    ...
    def render_c_hgvs(self, row: Dict[str, Any]):
        ...
        for index, genome_build in enumerate(self.genome_build_prefs):
            values.append({
                ...
                'value': row.get( VariantClassificationModification.column_name_for_build(genome_build) )
            })
        values.append({
            'type': 'imported',
        ...
        response = {
            'values': values,
            'variant_id': row.get('variant_classification__variant_id')
        }
    ...
    def classification_id(self, row: Dict[str, Any]):
        ...
        return {
            "id": row.get('variant_classification__id'),
            "lab_name": row.get('variant_classification__lab__name'),
            "lab_record_id": row.get('variant_classification__lab_record_id'),
            "share_level": row.get('variant_classification__share_level'),
            "matches": matches,
            "search": id_filter
        }
    ...
    def __init__(self, request):
        ...
        self.rich_columns = [
            RichColumn(
                key='variant_classification__id',
                sort_keys=['variant_classification__share_level', 'variant_classification__lab__name', 'variant_classification__lab_record_id'],
                name='id',
                label='ID',
                orderable=True,
            ...
            RichColumn(
                key=VariantClassificationModification.column_name_for_build(self.genome_build_prefs[0]),
            ...
            RichColumn(
                key='variant_classification__user__username',
                name='user',
                label='User',
            ...
            RichColumn(
                key='variant_classification__created',
                name='created',
                label='Created',
                orderable=True,
            ...
            RichColumn(
                key='variant_classification__flag_collection_id',
                name='flags',
                label='Flags',
        ...
        self.extra_columns = [
            "variant_classification__chgvs_grch37",
            "variant_classification__chgvs_grch38",
            'published_evidence__c_hgvs__value',
            'published_evidence__p_hgvs__value',
            'published_evidence__genome_build__value',
            'variant_classification__variant_id',
            'variant_classification__id',
            'variant_classification__lab__name',
            'variant_classification__lab_record_id',
            'variant_classification__share_level'
        ]
    ...
    def get_initial_queryset(self):
        ...
        initial_qs = VariantClassificationModification.latest_for_user(
            user=self.user,
            published=True,
        ...
        for genome_build in self.genome_build_prefs:
            column_name = VariantClassificationModification.column_name_for_build(genome_build)
    ...
    def filter_queryset(self, qs):
        ...
        if settings.VARIANT_CLASSIFICATION_GRID_SHOW_USERNAME:
            ...
            if user_id:
                filters.append(Q(variant_classification__user__pk=user_id))

        lab_id = self.get_query_param('lab')
        if lab_id:
            filters.append(Q(variant_classification__lab__pk=lab_id))
        ...
        if flags:
            ...
            filters.append(Q(variant_classification__flag_collection__in=flag_collections))
        ...
        if genes:
            ...
            match_gene = Q(variant_classification__variant__variantallele__allele__in=allele_qs)
            evidence_q_list = []
            symbols = set()
            for gene in genes:
        ...
        if issues_type:
            ...
            if issues_type == 'data':
                ...
                flag_types = [
                    variant_classification_flag_types.discordant,
                    variant_classification_flag_types.internal_review,
                    variant_classification_flag_types.significance_change,
                    variant_classification_flag_types.suggestion
                ]
            elif issues_type == 'withdrawn':
                flag_types = [
                    variant_classification_flag_types.variant_classification_withdrawn
                ]
            ...
            filters.append(Q(
                variant_classification__in=Subquery(vcqs.values('pk')),
            ))

        # VariantClassificationListGrid is also used on patient/sample page so we
        # need to filter by samples
        sample_ids = self.get_query_json("sample_ids")
        if sample_ids:
            filters.append(Q(variant_classification__sample__in=sample_ids))
        ...
        if protein_position:
            ...
            filters.append(Q(variant_classification__variant__variantallele__allele__variantallele__variant__in=variant_qs))
...
class VariantClassificationModificationDatatableView(BaseDatatableView):

    def config(self, request):
        return VariantClassificationDatatableConfig(request)
