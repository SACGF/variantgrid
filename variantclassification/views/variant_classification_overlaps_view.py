...
from variantclassification.models import VariantClassificationRef
from variantclassification.models.allele_overlap import AlleleOverlap, DiscordanceLevel, OverlapCounts
...
def view_overlaps(request: HttpRequest) -> Response:
    ...
    return render(request, "variantclassification/overlaps.html", context)
...
def post_clinical_context(request: HttpRequest) -> Response:
    ...
    for key, cc_name in request.POST.items():
        ...
        if match:
            ...
            vcid = int(match[1])
            vc = VariantClassification.objects.get(pk=vcid)
            ...
            if old_cc is None or old_cc.name != cc_name:
                ...
                vc.flag_collection_safe.add_flag(
                    flag_type=variant_classification_flag_types.variant_classification_clinical_context_changed,
                    user=request.user,
...
def view_clinical_context(request: HttpRequest, pk: int) -> Response:
    ...
    if last_evaluation:
        ...
        if considers_vcms is not None:
            try:
                records = [VariantClassificationRef.init_from_str(user=user, id_str=vcm_id).modification for vcm_id in considers_vcms]
    ...
    return render(request, 'variantclassification/clinical_context.html', context)
