...
from variantclassification.models.evidence_key import EvidenceKeyMap, EvidenceKey
...
class UsedKeyTracker:
    ...
    def check_record(self, vcm: VariantClassificationModification):
        has_value = False
        has_note = False
    ...
    def row(self, variant_classification_modification: VariantClassificationModification) -> List:
        cols = []
        evidence = variant_classification_modification.get_visible_evidence(self.user)
        for used_key in self.ordered_keys:
...
class VariantWithChgvs:
    ...
    def __init__(self, vcm: VariantClassificationModification, chgvs: CHGVS):
        self.vcm = vcm
        self.chgvs = chgvs
...
class AlleleGroup:
    ...
    def __init__(self, source: 'ExportFormatter', allele_id: int, genome_build: GenomeBuild):
        ...
        self.variant_ids = []
        self.data: List[VariantClassificationModification] = []
        self.source = source

    def filter_out_transcripts(self, transcripts: Set[str]) -> List[VariantClassificationModification]:
        passes: List[VariantClassificationModification] = []
        fails: List[VariantClassificationModification] = []
    ...
    def liftover(self, vcm: VariantClassificationModification) -> CHGVS:
        chgvs_str = vcm.variant_classification.get_c_hgvs(self.genome_build, use_full=self.source.use_full_chgvs)
        if not chgvs_str:
            raise ValueError(f"Unable to generate c.hgvs full={self.source.use_full_chgvs}, this record should have been filtered out")
        return CHGVS(vcm.variant_classification.get_c_hgvs(self.genome_build, use_full=self.source.use_full_chgvs))

    def iter_c_hgvs(self) -> Iterable[Tuple[CHGVS, List[VariantClassificationModification]]]:
        by_transcript: Dict[CHGVS, List[VariantClassificationModification]] = defaultdict(list)
    ...
    def iter_c_hgvs_versionless_transcripts(self) -> Iterable[Tuple[CHGVS, VariantWithChgvs]]:
        ...
        for vcm in self.data:
            ...
            if c_parts:
                ...
                report_message('MVL export : Could not liftover', extra_data={'imported_chgvs': vcm.get(SpecialEKeys.C_HGVS), 'id': vcm.variant_classification_id})
...
class ExportFormatter(BaseExportFormatter):
    ...
    def preferred_chgvs_column(self) -> str:
        return VariantClassificationModification.column_name_for_build(self.genome_build, use_full=self.use_full_chgvs)
    ...
    def record_and_error(self):
        for vcm in VariantClassificationModification.objects.filter(pk__in=self.error_message_ids.keys()).order_by('created'):
    ...
    def passes_flag_check(self, vcm: VariantClassificationModification) -> bool:
        outstanding_warning = Flag.objects.filter(
            collection_id=vcm.variant_classification.flag_collection_id,
            flag_type__in=[variant_classification_flag_types.matching_variant_warning_flag, variant_classification_flag_types.transcript_version_change_flag],
            resolution__status=FlagStatus.OPEN
        ).exists()
    ...
    def record_errors(self, ids: Union[QuerySet, int, List[int], VariantClassificationModification], message: str):
        if isinstance(ids, QuerySet):
            ids = ids.values_list('id', flat=True)
        elif isinstance(ids, int):
            ids = [ids]
        elif isinstance(ids, VariantClassificationModification):
            ids = [ids.id]
    ...
    def prepare_groups(self):
        ...
        all_variants = self.qs.values_list('variant_classification__variant', flat=True).distinct()
        ...
        if allele_warning_variant_ids:
            allele_warning_variant_qs = self.qs.filter(variant_classification__variant__in=allele_warning_variant_ids)
    ...
    def iter_group_by_allele(self):
        for allele_group in self.allele_groups:
            ...
            allele_group.data = [vcm for vcm in self.qs.filter(variant_classification__variant__in=allele_group.variant_ids) if self.passes_flag_check(vcm)]
    ...
    def filename(self) -> str:
        return 'variant_classifications.txt'

    def is_discordant(self, vc: VariantClassification):
