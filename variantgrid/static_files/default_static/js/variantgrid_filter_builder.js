/* Column filter builder for the variant grids.
   ...
   which is what filter_rules_to_q turns into a Q object, what a grid request carries as a
   'filters=<json>' param, and what FilterNode persists as FilterNodeItem rows. Fields, their types
   and their choices all come from the definition JSON's filterBuilder block
   (@see DatatableConfig.filter_builder_fields), so the list is whatever the grid can filter on.

   Two callers:
     - DataTableDefinition puts one behind the "Filter grid..." toolbar button on any grid whose
       definition carries a filterBuilder block, and sends the rules with the table's ajax params.
