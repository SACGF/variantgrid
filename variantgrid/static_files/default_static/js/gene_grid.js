...
const EVIDENCE_COLUMN_INFO = {
    "gene-disease": {
        "icon": "gencc-icon",
        "initFunc": geneDiseaseRelationshipCellInit,
    },
};

let ENRICHMENT_KIT_COLUMNS;
let ENRICHMENT_KIT_COLUMN_TOOL_TIPS;
let ENRICHMENT_KIT_COLUMN_LABELS;
let ENRICHMENT_KIT_COLUMN_LABEL_TOOL_TIPS;
let geneGridCallbacks = {};
...
function updateUrl() {
    let updateUrlCallback = geneGridCallbacks.updateUrl;
...
function getGeneListColumn(geneListId, selectElement, addEmptyGeneLists) {
    ...
    geneListId = String(geneListId);
    const className = 'gene-list-' + geneListId;
    removeColumn(className); // Remove any existing if this is a reload

    // geneListId can contain prefixes to distinguish special case ones eg panel app etc - so can store in same hash
    const SPECIAL_CASE_GENE_LISTS = {
        "panel-app-": Urls.api_view_panel_app_gene_list,
        "ontology-": Urls.api_ontology_term_gene_list,
    };

    let gene_list_url;
    for (const [prefix, get_url_func] of Object.entries(SPECIAL_CASE_GENE_LISTS)) {
        if (geneListId.startsWith(prefix)) {
            let pk = geneListId.replace(prefix, "");
            gene_list_url = get_url_func(pk);
            break;
        }
    }

    if (!gene_list_url) {
        gene_list_url = Urls.api_view_gene_list(geneListId);
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            let closeButtonFunc = function() {
                ...
                let closeButtonCallback = geneGridCallbacks.closeButton;
...
function populateGeneColumn(geneListId, data, className, saveGeneListFunc, closeButtonFunc, geneClickHandler) {
    ...
    if (can_write) {
        if (saveGeneListFunc) { 
            let saveButton = $("<button>save</button>").addClass("save-genelist btn btn-secondary");
            saveButton.hide();
            iconContainerDiv.append(saveButton);
            
            let saveGeneListCallback = geneGridCallbacks.saveGeneList;
...
function createRowColumns(tr, headerRow) {
    ...
    if (!geneType || geneType === 'gene' || geneType === 'unmatched') {
        let canAddGene = geneType != 'unmatched';
        columns = columns.concat(getGeneRowColumns(geneSymbol, headerRow, canAddGene));
    } else {
...
function getGeneRowColumns(geneSymbol, headerRow, canAddGene) {
    let columns = [];
    ...
    for (let i=0 ; i<geneAnnotationReleases.length ; ++i) {
        ...
        (function() { // capture
            ...
            if (geneIdList) {
                ...
                columnData["initFunc"] = function(cell, geneSymbol) {
                    let coverageDiv = $("<div />").addClass("loading icon16");
    ...
    for (let i=0 ; i<pathologyTestVersions.length ; ++i) {
        (function() {
            ...
            if (canAddGene && canWriteGeneList(geneListId)) {
...
function addSubColumns(thead, enrichmentKitId) {
    ...
    for (let c=0 ; c<ENRICHMENT_KIT_COLUMNS.length ; c++) {
        ...
        let className = enrichmentKitClass + "-" + columnName;
        setCell(tr, className, th);
    }
}
...
function addGeneDataRows(tbody, geneListId, geneListClass, geneDataList, geneClickHandler) {
    ...
    for (let i=0 ; i<geneDataList.length ; ++i) {
        ...
        let addCellClickHandler = geneListId;
        ...
        if (rowType === 'gene' || rowType === 'unmatched') {
            if (rowType === 'gene') {
                ...
                geneLabelDiv.append($("<span/>").text(gene).attr("title", "Could not match gene symbol"));
            }
            if (addCellClickHandler) {
                setCellGene(tr, geneListId, geneListClass, gene, geneClickHandler);
            }
...
function handleBatchRetrieveGoldCoverage(enrichmentKitId, goldCoverageGenesRequests, geneSymbols, data) {
    ...
    for (let i=0 ; i<data.length; ++i) {
        ...
        let geneSymbol;
        if (coverageData.gene_symbol) {
            geneSymbol = coverageData.gene_symbol.symbol;
        } else {
            geneSymbol = coverageData.original_gene_symbol;
        }
...
function setGeneAnnotationReleaseCell(cell, geneIdList) {
    ...
    if (geneIdList.length) {
        ...
        let warningMessage = $("<i class='fas fa-exclamation-circle'></i>");
        warningMessage.attr("title", "This symbol was not matched to a release symbol, no filtering will be performed");
        warningMessage.appendTo(cell);
    }
}
...
function handleBatchRetrieveGeneIdentifiersForRelease(releaseId, geneIdRequests, geneSymbols, returnedGeneIdentifiers) {
    ...
    console.log("handleBatchRetrieveGeneIdentifiersForRelease");
    console.log("gene symbols:");
    console.log(geneSymbols);
    console.log("returned identifiers:");
    console.log(returnedGeneIdentifiers);
...
function geneDiseaseRelationshipCellInit(cell, gene_symbol) {
    let geneDiseaseDiv = $("<div />").addClass("loading icon16");
    cell.append(geneDiseaseDiv);
    // make API call and when done - replace with this
    $.ajax({
        type: "GET",
        url: Urls.api_view_gene_disease_relationship(gene_symbol),
        success: function(diseaseRelationshipList) {
            let newDiv = $("<div />");
            if (diseaseRelationshipList) {
                for (let i=0 ; i<diseaseRelationshipList.length ; ++i) {
                    let diseaseRelationship = diseaseRelationshipList[i];
                    let ontologyTerm = diseaseRelationship["source_term"];
                    let diseaseLink = $("<a />", {href: ontologyTerm["absolute_url"], target: "_blank"})
                        .addClass("disease-name")
                        .text(ontologyTerm["name"])
                        .attr("title", ontologyTerm["definition"]);
                    let dv = $("<div />").addClass("disease-validity");
                    dv.append(diseaseLink);
                    newDiv.append(dv);

                    // Data is grouped and put into extra.sources
                    let listGroup = $("<ul />", {class: "list-group"});
                    let sources = diseaseRelationship["extra"]["sources"];
                    for (let j=0; j<sources.length ; ++j) {
                        let src = sources[j];
                        let classification = $("<span />")
                            .addClass("gene-disease-classification")
                            .text(src["mode_of_inheritance"]);

                        let classificationSummary = $("<span />").attr("title", src["evaluated_date"]);
                        classificationSummary.append(classification);
                        classificationSummary.append(" (" + src["gencc_classification"] + ") by " + src["submitter"]);

                        if (src["public_report_url"]) {
                            let reportLink = $("<a />").attr("href", src["public_report_url"]);
                            reportLink.append(classificationSummary);
                            classificationSummary = reportLink;
                        }
                        let li = $("<li/>", {class: "list-group-item"});
                        li.append(classificationSummary);
                        listGroup.append(li)
                    }
                    newDiv.append(listGroup);
                }
            }
            geneDiseaseDiv.replaceWith(newDiv);
        },
        error: function() {
...
function panelAppCellInit(cell, server_id, geneSymbol) {
    let panelAppDiv = getPanelAppGeneEvidenceDiv("", server_id, geneSymbol);
...
function pathologyTestModifications(pathologytestgenemodificationrequest_set) {
    ...
    for (let i=0 ; i<pathologytestgenemodificationrequest_set.length ; ++i) {
        ...
        let geneSymbol = mod["gene_symbol"]["symbol"];
...
function createPathologyTestGeneModificationForm(container, headerText, submitFunc) {
    ...
    let submitButton = $("<button />").addClass("btn btn-secondary").html("save").click(function() {
        resetStuff();
        submitFunc(input.val());
    }); 
    let cancelButton = $("<button />").addClass("btn btn-secondary").html("cancel").click(resetStuff);
...
function geneGridInit(config, callbacks) {
    geneGridCallbacks = callbacks;
    ENRICHMENT_KIT_COLUMNS = config.enrichment_kit_columns;
    ENRICHMENT_KIT_COLUMN_TOOL_TIPS = config.enrichment_kit_column_tool_tips;
    ENRICHMENT_KIT_COLUMN_LABELS = config.enrichment_kit_column_labels;
    ENRICHMENT_KIT_COLUMN_LABEL_TOOL_TIPS = config.enrichment_kit_column_label_tool_tips;

    for (const server of config.panel_app_servers) {
        EVIDENCE_COLUMN_INFO["panelapp-" + server.pk] = {
    ...
    for (const formId of config.panel_app_form_ids) {
    ...
    function ontologyTermChange() {
        const PREFIX = "ontology-";
        let ontology_term_id = $(this).val();
        if (ontology_term_id) {
            let ontology_slug = ontology_term_id.replace(":", "_");
            let geneListId = PREFIX + ontology_slug;
            addAndRetrieveGeneListColumnIfNotExists(geneListId, this);
        }
    }

    $('#id_hpo').change(ontologyTermChange);
    $('#id_omim').change(ontologyTermChange);
    ...
    $("button", "#custom-gene-list").click(addCustomGeneList);
    ...
    for (const column of config.initial_columns) {
