"""
The event log: Event (severity, app, name, details), ViewEvent (one row per page view) and
IntegrationActivity (a rolling last-seen per external system, updated in place through
`IntegrationActivity.track`). Write events with create_event / create_login_event; the logging
handler and middleware in this app fill the rest.
"""
import inspect
import logging
