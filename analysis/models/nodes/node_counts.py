...
REQUIRED_FIELDS = [
    ...
    "variantannotation__gene__geneannotation__omim_phenotypes",
...
COUNTS = {
    ...
    BuiltInFilters.OMIM: "sum(case when %(annotation_geneannotation)s.omim_phenotypes is not null then 1 else 0 end)",
...
def get_extra_filters_q(user: User, genome_build: GenomeBuild, extra_filters):
    if extra_filters == BuiltInFilters.CLINVAR:
        ...
        q = Q(variantannotation__gene__geneannotation__omim_phenotypes__isnull=False)
