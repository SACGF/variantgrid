...
class AbstractZygosityCountNode(Model):
    ...
    def get_zygosity_count_arg_q_dict(self) -> Dict[Optional[str], Set[Q]]:
        COUNT_COLUMNS = [
            # column                         MIN                 MAX
        ...
        arg_q_dict = defaultdict(set)
        for column, min_count, max_count in COUNT_COLUMNS:
            if min_count:
                arg_q_dict[column].add(Q(**{column + "__gte": min_count}))
            if max_count:
                arg_q_dict[column].add(Q(**{column + "__lte": max_count}))
        return arg_q_dict
