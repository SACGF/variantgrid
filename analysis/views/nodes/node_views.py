...
from library.django_utils import highest_pk, lookup_field_path
    ...
    def post(self, request, *args, **kwargs):
        ...
        if filters_data:
            ...
            for i, rule in enumerate(filters['rules']):
                ...
                if op == "eq":
                    # The filter builder sends '' for a rule with no value. Some fields can't deal with
                    # that - so in those cases we convert "equals blank" to "is null"
                    django_field = lookup_field_path(opts, field)
