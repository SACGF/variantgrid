...
def get_analysis_or_404(user: User, analysis_id) -> Analysis:
...
def get_node_subclass_or_404(user: User, node_id, **kwargs) -> AnalysisNode:
    """ Ensures user has view permission on analysis """
    try:
...
def get_node_subclass_or_non_fatal_exception(user: User, node_id, **kwargs):
    try:
        node = AnalysisNode.objects.get_subclass(pk=node_id, **kwargs)
        node.analysis.check_can_view(user)
        return node
