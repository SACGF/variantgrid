...
# known set. node_grid_export builds the export task's params from this list too, so the export sees
# the same filters as the grid the user was looking at.
...
class NodeGridHandler(NodeJSONViewMixin):
    ...
    def _get_data(self, request, node, **kwargs):
        ...
        return datatable_data(_variant_grid_from_request(request, node))
...
class NodeGridConfig(NodeJSONGetView):
    ...
    def _get_data(self, request, node, **kwargs):
        ...
        if errors:
            ...
            grid = grids.VariantGrid(request, node, kwargs["extra_filters"])
            ret = datatable_definition(grid)
            ret["postData"] = grid.post_data
        return ret
