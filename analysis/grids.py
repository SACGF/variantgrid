...
from snpdb.grid_columns.variant_columns import column_by_name, column_overrides, format_rows
...
class VariantGrid(AbstractVariantGrid):
    """ The analysis node grid """
    grid_name = 'VariantGrid'
    GENOTYPE_COLUMNS_MISSING_VALUE = "."
    SOURCE_COLUMN = "vcf_source"
    # The grid itself gets no "Filter grid..." button - filtering an analysis is what FilterNode is
    # for - but its editor mounts a builder off the definition's fields and operations
    filter_builder_toolbar = False
    # Whether to fetch rows at all is the page's call (the row count placeholder, and the grid tab
    # being hidden) - node_data_grid.html triggers the first load
    defer_loading = True
    # A node export can take minutes, so it runs under Celery off its own toolbar button rather than
    # streaming from this URL @see analysis.views.views_grid.node_grid_export
    server_csv_download = False
    node_column_overrides = {
        'tags': {'css_class': 'no-word-wrap', 'client_renderer': 'VariantGridFormat.tags', 'orderable': False},
    }
    ...
    def __init__(self, request: HttpRequest, node, extra_filters=None, af_show_in_percent=None):
        ...
        if self.sorting_disabled():
            for rich_column in self.rich_columns:
                rich_column.orderable = False

        self.post_data = self._get_post_data(node, extra_filters)
    ...
    def _setup(self):
        ...
        try:
            node_count = NodeCount.load_for_node(node, self.extra_filters)
        ...
        if not self.sorting_disabled():
            if default_sort_by_column := node.analysis.default_sort_by_column:
                self.default_sort_field = default_sort_by_column.variant_column
    ...
    def _get_post_data(self, node, extra_filters) -> JsonObjType:
        """ The per request state the data endpoint needs, which the page sends as its ajax params """
        post_data = node.get_extra_grid_config().get('postData', {})
    ...
    def get_known_count(self, qs) -> Optional[int]:
        """ The node load pipeline has already counted this node version, so the view doesn't have to
            count the annotated grid queryset again """
    ...
    def get_column(self, column_name: str) -> RichColumn:
        if rich_column := column_by_name(self.enabled_columns, column_name):
            return rich_column

        msg = f"{column_name} not found in grid columns"
        raise PermissionDenied(msg)

    def _get_fields_and_overrides(self) -> tuple[list, dict]:
        node = self.node
        af_show_in_percent = self.af_show_in_percent
        ...
        update_dict_of_dict_values(overrides, self.node_column_overrides)
        update_dict_of_dict_values(overrides, node.get_extra_column_overrides())
        if self.cohorts:
    ...
    def get_source_columns_and_overrides(cohorts, visibility):
        ...
        overrides = column_overrides(column_names, [col_data_dict], model_field=False, queryset_field=False)
        return column_names, overrides

    @staticmethod
    def _get_packed_column_sort_annotation(cohortgenotype_alias: str, sql_index: int, column: str):
        """ A sample's slot in its cohort's packed genotype column - what sorting on it orders by """
        ...
        if is_array:
            ...
            return F(f"{cohortgenotype_alias}__{column}__{sql_index - 1}")
        # Is string...
        return Substr(f"{cohortgenotype_alias}__{column}", sql_index, length=1)
        ...
        for sample, (cohort, cohort_index) in sample_cohort_index.items():
            for column, (column_label, label_format, width) in sample_columns.items():
                ...
                col_data_dict = {
                    ...
                    # There's no field to sort on - the value lives in a slot of the packed column
                    "sort_annotation": VariantGrid._get_packed_column_sort_annotation(cgc.cohortgenotype_alias,
                                                                                      sql_index, column),
                }
                column_data.append(col_data_dict)

        overrides = column_overrides(column_names, column_data, model_field=False, queryset_field=False)
        return column_names, overrides
...
class ExportVariantGrid(VariantGrid):
    ...
    def export_rows(self) -> Iterator[dict]:
        """ Every row, formatted, in genome build contig then position order """
        qs = self.filter_queryset(self.get_initial_queryset())
        return format_rows(self.enabled_columns, self._iter_by_pk_batches(qs))

    def _iter_by_pk_batches(self, qs) -> Iterator[dict]:
        ...
        values_qs = qs.values(*self.value_columns())
        ...
        for contig in self.node.analysis.genome_build.standard_contigs:
            ...
            for batch in iter_fixed_chunks(contig_pks, self.EXPORT_PK_BATCH_SIZE):
                batch_items = values_qs.filter(pk__in=batch).order_by("locus__position", "pk")
...
class NodeColumnSummaryConfig(DataFrameDatatableConfig):
    ...
    def __init__(self, request: HttpRequest):
        ...
        rich_column = grid.get_column(self.variant_column)
        self.grid_column_name = rich_column.label
        self.formatter = rich_column.server_side_formatter
