# #1361 / #1844 — Duplicate `VariantAllele`s per (variant, build)

Written by Claude Fable 5.1 (claude-fable-5-1), 2026-09-08 — revising the 2026-08-31 plan by an earlier Claude model.
Status: in progress
Verified against d485dc18c
...
[#1844](https://github.com/SACGF/variantgrid/issues/1844) is the same bug found again from the other end:
tagging a 43 kb `<DEL>` in analysis 48 on `vg-test2` made a second `Allele`/`VariantAllele` two seconds later,
and the variant then drew twice in the node grid and the tags node. One plan covers both.

Reproduced (rolled back) with a 36,005 bp GRCh38 deletion:
...
The dev DB holds **66** duplicate `(variant, genome_build)` groups (all pairs) — profiled in §2.4, re-measured
2026-09-08 with Appendix A.

---

## 1. Root cause

### 1.1 The leak — `snpdb/clingen_allele.py:populate_clingen_alleles_for_variants`
...
A variant that can *never* get a ClinGen allele (`snpdb/models/models_variant.py:Variant.clingen_allele_skip_reason`:
gene-level, un-HGVS-able symbolic, or over `ClinGenAllele.CLINGEN_ALLELE_MAX_ALLELE_SIZE = 10_000` bp, doubled
for DUPs) gets an empty `Allele` on the first call and then matches the "missing ClinGen" filter forever
(`clingen_error` null, no `clingen_allele`). Every later call routes it back to the skip branch, which — unlike
the API-error and API-success branches, which check `existing_va` — ignores the existing row and bulk-creates a
fresh `Allele` plus `VariantAllele`.

#1361's variant is a 10,001 bp deletion, one over the limit; #1844's is a 43,365 bp deletion.

### 1.2 Why the constraint doesn't catch it

`VariantAllele.Meta.unique_together = ("variant", "genome_build", "allele")`
(`snpdb/models/models_variant.py:VariantAllele`). The `allele` is brand new each time, so
`bulk_create(..., ignore_conflicts=True)` never conflicts.
...
per-event: `analysis/tasks/variant_tag_tasks.py:variant_tag_created_task`,
`analysis/tasks/analysis_update_tasks.py:populate_clingen_alleles_from_analysis_node` (every TagNode update),
`classification/signals/classification_liftover.py:liftover_run_complete_handler`, classification imports,
`liftover_alleles`, `upload/vcf/bulk_clingen_allele_vcf_processor.py`, and two management commands. One
duplicate per call.

### 1.4 Symptoms

- **Tag liftover dies.** `analysis/tasks/variant_tag_tasks.py:_liftover_variant_tag` does
...
- **Tag invisible in the other build.** `analysis/models/models_variant_tag.py:VariantTag.get_for_build`
  matches tags through `VariantAllele` on `allele`. A tag with `allele=None` is only visible in its own build
  via the native-build fallback, and never in the other build.
- **Grid rows multiply (#1844).** `snpdb/grids.py:AbstractVariantGrid` restricts the `variantallele` join to
  the grid's build (#1626), which stops cross-build doubling but not within-build doubling: two `VariantAllele`
  rows in one build put the variant in every grid twice.
- **Different code picks different alleles.** `snpdb/models/models_variant.py:Variant.allele` is
  `variantallele_set.first()` with no ordering, so `classification/models/classification.py:Classification.update_allele`
  and `classification/models/classification_variant_info_models.py:ImportedAlleleInfo.set_variant_and_save` may
  land on a different duplicate than the tag did.
...
- `snpdb/clingen_allele.py:link_allele_to_existing_variants` already does
  `get_or_create(variant=, genome_build=)` and merges when the allele differs.
- `upload/vcf/bulk_allele_linking_vcf_processor.py:BulkAlleleLinkingVCFProcessor.batch_handle_variant_ids`
  looks up existing links keyed on `(variant, build)`, merges, and comments *"Only one VariantAllele allowed"*.
- `snpdb/clingen_allele.py:get_variant_allele_for_variant` apologises for the race the wide constraint permits.

The docstring's "multiple variants → same allele" case is the *other* direction and stays legal. The
"same variant, several builds" case (MT / shared contigs) is distinct `genome_build` values and stays legal.
...
|---|---|---|---|
| `populate_clingen_alleles_for_variants` skip branch | leaks (§1.1) | still leaks an orphan `Allele` per call; the VA insert is now silently ignored | **fix the branch** (§3.1) |
| same function, API-error / API-success new-VA branches | dupe only in a race | race loser's VA ignored; error-branch loser leaves an orphan empty `Allele` | delete unlinked new alleles after the bulk_create (§3.1) |
| `get_variant_allele_for_variant` no-ClinGen branch | dupe only in a race | race loser raises `IntegrityError` | catch, drop the just-made `Allele`, re-fetch (§3.2) |
| `snpdb/clingen_allele.py:variant_allele_clingen` error branch | dupe only in a race | as above | same helper (§3.2) |
| `link_allele_to_existing_variants` | `get_or_create` + merge | unchanged | none |
| `snpdb/liftover.py:_run_liftover_using_same_contig` | if the dest-build variant already has its own allele, adds a second link | insert silently dropped, `AlleleLiftover` still marked `SUCCESS`, alleles never merged | **merge like the bulk processor does** (§3.3) |
| `BulkAlleleLinkingVCFProcessor` | detects existing, merges | unchanged | none |
| `analysis/fake_variant_tags.py` `_create_alleles` (dev only) | picks variants without `without_alleles=True`, then makes a new `Allele`+VA for each — 57 of the 66 dev dupes | `bulk_create` (no `ignore_conflicts`) raises | **pick only unlinked variants** (§3.4) |
| `classification/fake_reclassifications.py` `_create_allele_infos` (dev only) | already passes `without_alleles=True` | unchanged | none |
| `upload/tasks/import_variant_tags_task.py` | reads VAs into `dict(variant_id → allele_id)` — with dupes an arbitrary one wins | one row per key, deterministic | none |

Test fixtures were audited (§4): none links one variant/build twice.

### 2.3 Orphan `Allele`s

`snpdb/models/models_variant.py:Allele.merge` already leaves the merged-away allele in place with no
...
Profile of the 66 dev-DB groups (all size 2, all `origin='D'`), re-measured 2026-09-08:
...
Surplus alleles are referenced by `VariantTag` in 65 groups and `AlleleLiftover` in all 66; none by a
`Classification` or `ImportedAlleleInfo` on this box, but the dedupe handles those FKs anyway because other
deployments will differ. No group has two ClinGen alleles. `VariantTag.allele` is `None` on 0 tags here
(#1361's deployment has at least one).
...
FKs to `Allele` (from `grep ForeignKey(Allele`, re-run 2026-09-08 — unchanged):
...
`ImportedAlleleInfo.allele` on save. Clinical contexts are re-homed lazily by
`classification/models/clinical_context_utils.py:_update_clinical_context` and groupings by
`classification/models/classification_grouping.py:ClassificationGrouping.assign_grouping_for_classification`,
both driven by classification publish/withdraw/allele-info signals — so a bulk `.update(allele=keeper)` on
classifications leaves their `clinical_context` and `ClassificationGroupingEntry` pointing at the *old*
allele until something re-saves each one. §3.9 handles that.

### 2.5 Where the dedupe runs

The upgrader (`scripts/migrator/migrator.py`) runs `manage.py migrate` in the standard pass and only
...
drop any empty allele this call created that didn't end up linked — this is the race loser under the new
constraint:
...
(`allele_no_clingen_list` is consumed by `.pop()` — capture the ids before that loop, or keep a separate
list of created alleles.) Alleles with a `clingen_allele` are deduped by the OneToOne already and need no
cleanup.
...
`get_variant_allele_for_variant` and `variant_allele_clingen` both do `Allele.objects.create()` then
`VariantAllele.objects.create(...)`. Under the new constraint a concurrent creator makes the second call
raise `IntegrityError`. Extract one helper used by both:
...
can become a plain `.filter(...).first()` — leave the ordering, drop the comment.

### 3.3 Same-contig liftover must merge — `snpdb/liftover.py`

`snpdb/liftover.py:_liftover_using_existing_contig` only checks whether the *allele* already has a link in
the destination build, not whether the destination-build *variant* already has an allele of its own (e.g. an
MT variant imported directly into GRCh38 before the GRCh37 copy was lifted over). In
`_run_liftover_using_same_contig`, mirror `BulkAlleleLinkingVCFProcessor`:
...
### 3.4 Fake tag generator — `analysis/fake_variant_tags.py`

Pass `without_alleles=True` to `annotation/fake_data.py:get_variant_ids_by_gene`, as
`classification/fake_reclassifications.py` already does, so `_create_alleles` only ever makes an `Allele`
for a variant nothing has claimed. This is the writer behind 57 of the dev DB's 66 groups.

### 3.5 Extend `Allele.merge` to move the FKs it currently strands — `snpdb/models/models_variant.py`
...
(`AlleleGrouping.objects.get_or_create(allele=allele)` in `assign_grouping_for_classification`) and both
carry uniqueness that a blind update could violate. Leave `AlleleMergeLog` as history.
...
**Savepoint.** The existing `VariantAllele` move does `try: va.save() except IntegrityError: va.delete()`
with no `transaction.atomic()` around the save. Under the new constraint that collision is the *normal*
case whenever both alleles already link the same variant in the same build (exactly the §3.3 scenario),
and an uncaught-at-DB-level `IntegrityError` inside an outer transaction (tests, `ATOMIC_REQUESTS`, a
celery task wrapped in `atomic`) leaves the connection in "current transaction is aborted" state for
everything after it. Wrap the save:
...
`liftover_run_complete_signal` in `snpdb/models/models_variant.py`:

```python
allele_merged_signal = django.dispatch.Signal()   # kwargs: old_allele, new_allele
```

sent at the end of a successful `merge()`. A receiver in `classification/signals/` (
classification_hooks_allele_merge.py, new) does, for
...
`classification/models/clinical_context_utils.py:update_clinical_contexts` already knows how to move a
classification to the matching context on its new allele and recalc discordance on both sides. The same
code is what §3.9's command runs, so put the "re-home these classifications" body in one function in
`clinical_context_utils` (or the command module) and call it from both.

### 3.6 Migration — new snpdb/migrations/0259_variantallele_unique_variant_build.py
...
Dependencies (heads as of 2026-09-08 — re-check at implementation time):
...
3. `ManualOperation(task_id=ManualOperation.task_id_manage(["classification_fix_allele_links"]),
   ...
   `ManualOperation` is in `manual/operations/manual_operations.py`.
...
### 3.7 Backfill `VariantTag.allele` — new analysis/migrations/0142_backfill_variant_tag_allele.py

Depends on `snpdb.0259`. `RunPython`:
...
Dev DB has 0 such tags; #1361's deployment has at least one. Tags whose variant has no `VariantAllele`
...
Backfilled tags become visible in their own build immediately (native-build match). The crashed task also
...
### 3.8 `_liftover_variant_tag` — `analysis/tasks/variant_tag_tasks.py`

Keep the `.get()`. After §3.6 it's correct and fails loudly if the invariant is ever broken again; the
code fix (§3.1) and the constraint ship in the same release, so there is no window in which a `.first()`
would be needed.
...
clinical-context predicate on historical models — the grouping path is optional in the test). The command
...
Add to `snpdb/tests/test_clingen_allele.py` (uses `get_fake_annotation_version` +
...
- **`test_populate_never_clingen_variant_is_idempotent`** — build a GRCh37 `<DEL>` with
  ...
  svlen=)` as in `snpdb/tests/test_variant.py`, then `Variant.objects.get_or_create` the way
  `snpdb/tests/utils/vcf_testing_utils.py:slowly_create_test_variant` does). Call
  `populate_clingen_alleles_for_variants` three times; assert one `VariantAllele` and one `Allele` for the
  variant. This is the regression test for both issues.
...
Add to `snpdb/tests/test_allele.py` (which already covers the happy path, self-merge and both-ClinGen refusal):
...
Add to `classification/tests/` (needs a classification fixture —
`analysis/tests/test_classifications_node.py:ClassificationTestDataMixin`):
...
**Fixture audit (re-run 2026-09-08).** Every `VariantAllele` created in tests
(`snpdb/tests/utils/vcf_testing_utils.py:create_mock_allele` and the direct `VariantAllele.objects.create`
calls in `snpdb/tests/test_variant_zygosity_preview_extra.py`, `snpdb/tests/test_variant.py`,
`snpdb/tests/test_allele.py`, `variantopedia/tests/test_variant_sample_information.py`) was checked: the
"same variant, two builds" cases use distinct `genome_build`s, and no test links one variant/build twice.
No fixture changes are needed for the new constraint.

Add to `snpdb/tests/test_liftover.py` next to `test_liftover_using_existing_variant`:
...
keeps the test DB schema — the first run after adding 0259 needs the migration applied to it, which
...
3. §3.6 migration, verify against the dev DB (66 groups of both flavours — a good fixture; run Appendix A
   before and after).
4. §3.7 backfill migration.
5. Tests in §4; full `snpdb`, `analysis`, `classification.tests.utils.test_urls`,
   `variantopedia.tests` runs. `scripts/vg map` (model constraint and new command changed).
6. Post-deploy check on the affected deployments: Appendix A returns 0 groups; the tag on
   `NC_000007.14:g.140753336_140763336del` shows in both builds (#1361); variant 16794059 draws once in
   analysis 48's grid and tags node on `vg-test2` (#1844).

## 6. Out of scope (noted for later)

- `populate_clingen_alleles_for_variants` excludes VAs with any `clingen_error` from the retry filter, so a
...
- `Variant.allele` uses an unordered `.first()`. After 0259 a variant has at most one allele per build,
