...
class DataFrameTableView(JSONResponseView):
    ...
    def download_csv(self) -> StreamingHttpResponse:
        df = self._sort(self._dataframe())
        columns = [{"name": key, "label": label} for key, label in self._column_labels(df)]
        return csv_streaming_response(self._csv_name(), grid_export_csv(columns, self._rows(df)))
