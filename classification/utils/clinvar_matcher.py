...
from genes.hgvs import HGVSComponents, HGVSException
...
class ClinVarLegacyRow:
    ...
    @property
    def variant_description(self) -> Optional[HGVSComponents]:
        if cd := self.variant_description_str:
            return HGVSComponents(cd).with_gene_symbol(self.submitted_gene)
    ...
    @property
    def c_hgvs_with_gene_symbol(self) -> Optional[HGVSComponents]:
        if c_hgvs_str := self.c_hgvs_preferred_str:
            return HGVSComponents(c_hgvs_str).with_gene_symbol(self.submitted_gene)
        return None
                ...
                if clinvar_exports := ClinVarExport.objects.filter(clinvar_allele=clinvar_allele):
                    for clinvar_export in clinvar_exports:
                        ...
                        if classification_based_on := clinvar_export.classification_based_on:
                            ...
                            # if HGVSComponents(classification_based_on.get(SpecialEKeys.C_HGVS)) == self.c_hgvs_with_gene_symbol:
