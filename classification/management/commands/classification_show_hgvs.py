        ...
        for vc in Classification.objects.filter(variant__isnull=False):
            ...
            if transcript:
                try:
                    ...
                    original_hgvs = matcher.variant_to_c_hgvs_parts(vc.variant, transcript, throw_on_issue=True).full_hgvs
                    for va in allele.variantallele_set.exclude(pk=orig_allele.pk):
                        matcher = HGVSMatcher.instance(va.genome_build)
                        converted_hgvs = matcher.variant_to_c_hgvs_parts(va.variant, transcript, throw_on_issue=True).full_hgvs
