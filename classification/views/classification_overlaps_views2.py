...
def view_overlaps_2(request: HttpRequest, lab_id=None) -> HttpResponseBase:
    lab_picker = LabPickerData.from_request(request, lab_id, 'overlaps_2')
    if redirect_response := lab_picker.check_redirect():
        return redirect_response

    return render(request, "classification/overlaps_2.html", {"lab_picker_data": lab_picker})
...
class OverlapView(AjaxFormView[OverlapContribution]):
    ...
    def lazy_render(cls, overlap: Overlap, context: Optional[dict] = None) -> LazyRender:
        ...
        return LazyRender(
            template_name="classification/overlap_detail.html",
            core_object=overlap,
            core_object_name="triage",
            static_context=context,
            dynamic_context=dynamic_context_gen
    ...
    def handle(self, request, overlap_id: int):
        # FIXME security checks
        triage = Overlap.objects.get(pk=overlap_id)
        classification_grouping = triage.classification_grouping
        value_type = triage.value_type
