...
class EvidenceMixin:
    ...
    def met_criteria_strengths(self) -> set[CriteriaStrength]:
        """ Met ACMG criteria plus each AMP level value as a custom strength, e.g. "A_<pretty sub value>" """
        from classification.models import EvidenceKeyMap
        ...
        for amp_level, letter in SpecialEKeys.AMP_LEVELS_TO_LEVEL.items():
            if value := self.get_value_list(amp_level):
                ...
                for sub_value in value:
                    ...
                    strengths.add(CriteriaStrength(ekey=e_key, custom_strength=f"{letter}_{sub_value_label}"))
        return strengths
