...
from classification.models import VCBlobKeys
from classification.models.tests.test_utils import VariantClassificationTestUtils
...
class VariantClassificationTestCaseModifications(TestCase):
    ...
    def ensure_latest_version(self, vc: VariantClassification):
        latest_ver = VariantClassificationRef.init_from_obj(vc.user, vc.last_edited_version)
        latest_json = latest_ver.as_json(VariantClassificationJsonParams(current_user=vc.user, include_data=True, flatten=True))['data']

        editable_json = vc.as_json(VariantClassificationJsonParams(current_user=vc.user, include_data=True, flatten=True))['data']
    ...
    def test_modifying(self):
        ...
        try:
            ...
            initial_version_qs = VariantClassificationModification.objects.filter(variant_classification=vc)
