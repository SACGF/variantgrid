...
from classification.models.clinical_context_models import DiscordanceNotification
...
def notify_discordance_change(discordance_report: DiscordanceReport, cause: str, cause_code: Optional[str] = None, **kwargs):
    if settings.DISCORDANCE_ENABLED:
        prepare_discordance_notification(discordance_report=discordance_report, cause=cause)
        if cause_code != 'ClinicalContextRecalcTrigger.DELAYED':
...
def send_prepared_discordance_notifications():
    labs_to_notify = DiscordanceNotification.objects.filter(notification_sent_date=None).values('lab').distinct()
    ...
    for lab_info in labs_to_notify:
        ...
        discordance_entries = DiscordanceNotification.objects.filter(lab=lab_obj, notification_sent_date=None)
